...
import type { NearCatalogDatabase } from "./index";
...
export async function migrate(db: NearCatalogDatabase, migrations: Migration[]): Promise<void> {
  ...
  for (const migration of migrations) {
    ...
    console.log(`[NearCatalog] Applying migration: ${migration.tag}`);
    ...
    for (const statement of migration.sql) {
      try {
        ...
        if (IDEMPOTENT_ERRORS.test(msg)) {
          console.log(
            `[NearCatalog] Skipping statement (already applied): ${statement.slice(0, 80)}`,
          );
          continue;
        }
        throw err;
