...
export async function migrate(db: ActivityDatabase, migrations: Migration[]): Promise<void> {
  ...
  for (const migration of migrations) {
    ...
    console.log(`[Activity] Applying migration: ${migration.tag}`);
    ...
    for (const statement of migration.sql) {
      try {
        ...
        if (IDEMPOTENT_ERRORS.test(msg)) {
          console.log(`[Activity] Skipping statement (already applied): ${statement.slice(0, 80)}`);
          continue;
        }
        throw err;
      }
