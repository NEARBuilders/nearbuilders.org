...
function proposalSearchLabel(status: CatalogClaimProposal["status"]) {
  ...
  if (status === "rejected") return "Use resubmit";
  return "Revoked";
}
...
function proposalConflictMessage(status: CatalogClaimProposal["status"]) {
  ...
  return "A revoked contribution cannot be resubmitted.";
}
...
function CatalogClaimProposalCard({
  ...
  const projectQuery = useQuery({
    queryKey: ["catalog-project", proposal.projectSlug],
    queryFn: () => apiClient.getCatalogProject({ slug: proposal.projectSlug }),
    retry: false,
  });
...
export function CatalogClaimFlow({ apiClient }: { apiClient: CatalogClaimApiClient }) {
  ...
  const proposalsQuery = useQuery({
    queryKey: ["catalog-claim-proposals"],
    queryFn: () => apiClient.getMyCatalogClaimProposals(),
    retry: false,
  });
    ...
    <div className="space-y-10">
      <form
        ...
        onSubmit={(event) => {
          ...
          if (
            existingProposal &&
            !(existingProposal.status === "rejected" && editingProposalId === existingProposal.id)
          ) {
      ...
      <section aria-labelledby="catalog-claim-status-heading" className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Track reviews and update any rejected submissions.
          </p>
        </div>
