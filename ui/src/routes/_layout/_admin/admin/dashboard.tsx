...
import { XNominationQueue } from "./-x-nomination-queue";
...
const TAB_ICONS = {
  ...
  activity: Activity,
  "x-nominations": XIcon,
} satisfies Record<DashboardTab, ComponentType<{ className?: string }>>;
...
function AdminDashboard() {
  ...
  const isXNominationTab = tab === "x-nominations";
  const status: DashboardStatus = search.status ?? "all";
  const pluginId = getPluginId(tab);
  const query = search.q ?? "";
  const [searchInput, setSearchInput] = useState(query);
  const activeTab =
    ADMIN_DASHBOARD_TABS.find((item) => item.value === tab) ?? ADMIN_DASHBOARD_TABS[0];
  ...
  const proposalsQuery = useInfiniteQuery(
    getProposalQueryOptions(apiClient, pluginId, status, query, !isXNominationTab),
  );
  ...
  const selectedQuery = useQuery({
    ...
    enabled: !isXNominationTab && Boolean(search.item) && !selectedLoadedProposal,
  });
    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      ...
      <Tabs value={tab} onValueChange={(value) => setTab(value as DashboardTab)} className="gap-0">
        <div className="overflow-x-auto">
          <TabsList aria-label="Admin review category" className="min-w-max">
            {ADMIN_DASHBOARD_TABS.map((item, index) => {
              const Icon = TAB_ICONS[item.value];
              const isXTab = item.value === "x-nominations";
              const proposalPendingQuery = isXTab ? undefined : pendingCountQueries[index];
              const pendingCount = proposalPendingQuery?.data;
              const pendingCountLoading = proposalPendingQuery?.isLoading;
              const pendingCountError = proposalPendingQuery?.isError;
              return (
      ...
      <ProposalReviewSheet
        open={!isXNominationTab && Boolean(search.item)}
        itemKey={search.item}
